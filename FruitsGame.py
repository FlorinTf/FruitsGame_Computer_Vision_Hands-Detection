...
while start:
    ...
    if timeRemain < 0:
        ...
        rectKivi.y -= speed +1
        rectLemon.y -= speed +1
        rectPepper.y -= speed -1
        rectPizza.y -= speed +8
        rectTomato.y -= speed-2
        ...
        if len(hands) == 2:
            ...
            # collide(rectPizza,-10)
            collide(rectAvocado,3)
            collide(rectWatermelon,1)

            # fourcc = cv.VideoWriter_fourcc(*'XVID')
            # out = cv.VideoWriter('./images/Web_cam' + str(1) + '.avi', fourcc, 3.0, (640, 480))
            # out.write(img)
        ...
        # window.blit(imgLemon,rectLemon)
        # window.blit(imgPepper,rectPepper)
        # window.blit(imgPizza,rectPizza)
        # window.blit(imgTomato,rectTomato)
